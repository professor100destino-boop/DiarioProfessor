...
self.addEventListener('message',e=>{
  if(e.data?.type==='PREPARE_OFFLINE'){
    e.waitUntil(prepare().then(async()=>{
      const cs=await self.clients.matchAll({includeUncontrolled:true});
      cs.forEach(c=>c.postMessage({type:'OFFLINE_READY'}));
    }).catch(async()=>{
      const cs=await self.clients.matchAll({includeUncontrolled:true});
      cs.forEach(c=>c.postMessage({type:'OFFLINE_ERROR'}));
    }));
  }
});
